...
_Authoring guidance for the `diagrams-mermaid` component — when to use it, how, and the rules._

Called as `c.diagrams_mermaid_diagram(...)`: this component sits flat at
`components/diagrams-mermaid/`, not under a `diagrams/` category, so it carries
the prefix its folder name gives it.

Styled by: `css/diagrams/diagrams.css` (shared viewport) +
`css/diagrams/diagrams-mermaid.css` (engine specifics)
...
displays its source as a readable code box (`diagrams-mermaid.css`). Once
