...
_Authoring guidance for the `charts-apache-echarts` component — when to use it, how, and the rules._

Called as `c.charts_apache_echarts_chart(...)`. This is the ENGINE, not a chart kind:
it sits flat at `components/charts-apache-echarts/`, beside `components/charts-apache-echarts/`
rather than inside it. **Reach for a kind first** — `c.charts_apache_echarts_bar`, `c.charts_apache_echarts_line`, `c.charts_apache_echarts_pie`
and eighteen others write the spec for you. Call this directly only for a shape
the kinds do not cover: candlestick/OHLC, boxplot, heatmap.

Styled by: `css/charts/charts.css` (the shared frame) +
`css/charts/charts-apache-echarts.css` (engine specifics)
Rendered by: the pinned Apache ECharts CDN script, loaded by the
`charts-apache-echarts` feature only when the document holds a matching element
...
```html
{% raw %}{% call c.charts_apache_echarts_chart(height=320) %}{% endraw %}
{
