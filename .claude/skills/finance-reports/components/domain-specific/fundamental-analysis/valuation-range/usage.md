...
- **`scale_min` / `scale_max` bound every bar.** A range that falls outside
  them is clipped silently, so set the axis last, after the ranges are known.
- **Each method names its key assumption in the label** — "WACC 8.5%, TGR
  3.0%", not just its name.
- **Four to six methods.** A field of ten bars says you have no view.
- **Include the 52-week range as the reality check.**
- **The price marker is spot at the `asof` date in `security_header`** — keep
  the two dates identical.
