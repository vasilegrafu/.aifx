...
- **The action is a verb the reader can execute today** — "Accumulate below
  $240", never "attractive" or "one to watch".
- **A price target without a horizon is not a target.** Always give both.
- **Conviction is a real input to sizing**, so state it and let
  `expected_value` or `scorecard` justify it.
- **The rationale is ONE sentence.** The argument itself belongs in
  `thesis_pillars`.
- **Revisit and re-date this block on every document update.** A stale call is
  worse than none.
