    ...
    init(nodes) {
      ...
      docsHtml.util.loadScript(ECHARTS).then(() => {
        ...
        jobs.forEach(({ pre, option, source }, i) => {
          ...
          // Mark the canvas as THIS engine's before it draws. The shared frame
          // owns `.chart-canvas`; the containment rules for the wrapper div
          // ECharts generates are true of ECharts and false of the next engine,
          // so they hang off this class instead of reaching into the shared one.
          frame.canvas.classList.add("charts-apache-echarts-canvas");
