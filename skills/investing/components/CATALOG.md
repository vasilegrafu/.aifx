...
[foundational](#foundational) 41 · [charts](#charts) 21 · [domain-specific](#domain-specific) 45 · [diagrams](#diagrams) 1 · [math](#math) 1
...
One analysis discipline owns these. Classes namespaced `investing-`.
...
| `attribution` | `attribution` | Brinson attribution: allocation effect, selection effect and their total by segment | [usage](domain-specific/fundamental-analysis/attribution/usage.md) |
...
| `bridge` | `bridge` | waterfall bridge decomposing how a total moved from one value to another | [usage](domain-specific/fundamental-analysis/bridge/usage.md) |
...
| `cohort-table` | `cohort_table` | cohort retention or vintage performance: cohorts down, periods since start across | [usage](domain-specific/fundamental-analysis/cohort-table/usage.md) |
| `composite-score` | `composite_score` | a formula-based composite score with its inputs, the result, and the band it falls in | [usage](domain-specific/fundamental-analysis/composite-score/usage.md) |
...
| `cycle-position` | `cycle_position` | where the economy or a market sits in a multi-phase cycle | [usage](domain-specific/fundamental-analysis/cycle-position/usage.md) |
...
| `drawdown-table` | `drawdown_table` | the worst drawdowns with peak, trough, recovery and how long each took | [usage](domain-specific/fundamental-analysis/drawdown-table/usage.md) |
...
| `expected-value` | `expected_value` | probability-weighted scenarios with the expected value total | [usage](domain-specific/fundamental-analysis/expected-value/usage.md) |
| `exposure-bars` | `exposure_bars` | allocation breakdown as labeled percentage bars — sector, geography, asset class | [usage](domain-specific/fundamental-analysis/exposure-bars/usage.md) |
...
| `funnel` | `funnel` | nested magnitudes narrowing to a target — market sizing, or a conversion funnel | [usage](domain-specific/fundamental-analysis/funnel/usage.md) |
| `heatmap` | `heatmap` | value grid with colour-graded cells — monthly returns, correlations, sector by region | [usage](domain-specific/fundamental-analysis/heatmap/usage.md) |
| `holdings-table` | `holdings_table` | portfolio positions with a weight bar, plus whatever columns the review needs | [usage](domain-specific/fundamental-analysis/holdings-table/usage.md) |
...
| `macro-indicators` | `macro_indicators` | economic indicators with latest, prior, consensus, surprise and direction | [usage](domain-specific/fundamental-analysis/macro-indicators/usage.md) |
| `metric-trend` | `metric_trend` | one metric per row across reporting periods, with CAGR and direction | [usage](domain-specific/fundamental-analysis/metric-trend/usage.md) |
...
| `performance-table` | `performance_table` | returns across periods for the portfolio, its benchmark and the excess | [usage](domain-specific/fundamental-analysis/performance-table/usage.md) |
| `quadrant-map` | `quadrant_map` | 2x2 positioning map with items placed by two scores | [usage](domain-specific/fundamental-analysis/quadrant-map/usage.md) |
...
| `risk-metrics` | `risk_metrics` | risk and return statistics side by side with the benchmark | [usage](domain-specific/fundamental-analysis/risk-metrics/usage.md) |
| `roll-forward` | `roll_forward` | movement schedule: opening balance, the movements, closing balance | [usage](domain-specific/fundamental-analysis/roll-forward/usage.md) |
| `scorecard` | `scorecard` | weighted multi-criteria score with per-criterion rating bar and weighted total | [usage](domain-specific/fundamental-analysis/scorecard/usage.md) |
...
| `sensitivity-table` | `sensitivity_table` | two-way sensitivity grid: one output across two assumptions, base case marked | [usage](domain-specific/fundamental-analysis/sensitivity-table/usage.md) |
| `stress-test` | `stress_test` | portfolio or company impact under named stress scenarios | [usage](domain-specific/fundamental-analysis/stress-test/usage.md) |
...
| `trade-log` | `trade_log` | dated trades with side, size, price, result and the rationale that justified them | [usage](domain-specific/fundamental-analysis/trade-log/usage.md) |
...
| `variance-analysis` | `variance_analysis` | budget versus actual with the variance and whether it is favourable | [usage](domain-specific/fundamental-analysis/variance-analysis/usage.md) |
